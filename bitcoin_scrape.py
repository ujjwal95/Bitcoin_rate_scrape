...
chrome_driver = '** Own driver location **'
browser = webdriver.Chrome(executable_path = chrome_driver, chrome_options= options)
# url = 'https://www.unocoin.com/trade?buy'
# browser.get(url)
# result_first = browser.find_element_by_xpath('/html/body').text

url = 'https://koinex.in/api/ticker'
browser.get(url)
result_second = browser.find_element_by_xpath('/html/body/pre').text
jdata = json.loads(browser.find_element_by_xpath('/html/body/pre').text)
data = jdata['prices']['XRP']
...
while True:
    ...
    jdata = json.loads(browser.find_element_by_xpath('/html/body/pre').text)
    data = jdata['prices']['XRP']
    balloon_tip('Ripple Rate',str(data))
    time.sleep(10)
